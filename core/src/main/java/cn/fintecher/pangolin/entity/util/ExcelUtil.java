...
public class ExcelUtil {
    ...
    public static Object parseRow(Class<?> dataClass, Row dataRow, int startCol, Map<Integer, String> headerMap, List<CellError> cellErrors, String sheetName, int rowIndex) {
                                ...
                                if (chineseCompare(cellName, titleName, "UTF-8")) {
                                    ...
                                    try {
                                        if (field.getName().equals("source")) {
                                            ESource eSource = ESource.getESourceByRemark(getCellValue(cell));
                                            if (eSource != null) {
                                                field.set(obj, eSource.getValue());
                                            }
                                            continue;
                                        }
                                        if (field.getName().equals("collectionType")) {
                                            ECollectionType eCollectionType = ECollectionType.getEnumByRemark(getCellValue(cell));
                                            if (eCollectionType != null) {
                                                field.set(obj, eCollectionType.getValue());
                                            }
                                            continue;
                                        }
                                        ...
                                        String errorMsg = "第[" + dataRow.getRowNum() + "]行，[" + cellName + "]的数据类型不正确";
        ...
        System.out.println(cellErrors.toString());
        System.out.println(cellErrors);
        return obj;
    }
                    ...
                    for (Field field : fields) {
                        ...
                        if (Objects.nonNull(result) && result.equals(field.getName())) {
                            ...
                            try {
                                ...
                                String errorMsg = "第[" + dataRow.getRowNum() + "]行，[" + (mapName.containsKey(field.getName()) ? mapName.get(field.getName()) : field.getName()) + "]的数据类型不正确";
                                CellError errorObj = new CellError(sheetName, rowIndex, colIndex, null, "titlename", errorMsg, e);
                                cellErrors.add(errorObj);
                            }
                            break;
        ...
        if (!dataList.isEmpty()) {
            ...
            for (int i = 0; i < dataList.size(); i++) {
                ...
                for (int k = 0; k < proNames.length; k++) {
                    ...
                    Object proValue = getProValue(exportItem, obj);
                    if (proValue != null) {
                        if (proValue instanceof Date) {
                            valueStr = ZWDateUtil.fomratterDate((Date) proValue, "yyyy/M/d");
                        } else if (proValue instanceof Integer) {
                            valueStr = String.valueOf(proValue);
                        } else if (proValue instanceof Double) {
                            valueStr = String.valueOf(proValue);
                        } else {
                            valueStr = (String) proValue;
                        }
                    }
